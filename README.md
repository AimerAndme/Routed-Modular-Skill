...
一种面向复杂 Agent Skill 的路由式模块化架构：一个轻量入口、一份模块索引、一组自包含模块，按当前场景加载文档并执行脚本。
...
skills/routed-modular-skill/
  SKILL.md                 # Agent 入口
  catalog.json             # 轻量路由索引
  scripts/rms.py           # list / load / run 参考实现
...
1. 宿主发现并激活 SKILL.md。
2. Agent 查看索引中的 id、when 和 guide，结合用户输入选择模块。
3. load 仅返回选中模块的指南，以及公共契约的路径。
4. Agent 按指南补全必要输入，run 执行已注册脚本。
5. 将结构化结果交给业务层校验、解释或汇总。
...
3. 在 catalog.json 添加唯一 id、简短 when、guide 和 entrypoint，所有路径相对 Skill 根目录。
...
- 未知模块、非法输入、非零退出、超时、无效输出均以结构化错误退出。
